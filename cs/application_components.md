...
API26, 오레오 이상부터는 백그라운드 서비스는 앱이 포그라운드에 있지 않으면 강제 종료되므로 포그라운드 서비스나 알람매니저,워크매니저의 사용이 필요하다
  
2. **Bound Service**
애플리케이션 구성 요소의 바인딩에 의해 실행되는 서비스이다. (또는 다른 구성요소가 바인딩할 수 있는 서비스라고도 할 수 있다.)
구성요소와 서비스는 각각 클라이언트-서버의 역할을 맡게 된다. 일반적으로 구성요소를 도울 때 까지만 실행된다.
startService()호출 이후 bindService()를 호출하여 started Service에 바인딩하는것도 가능하다.
이 경우 구성 요소의 바인딩이 모두 해제되어도 서비스가 동작하도록 유지하는 것이 가능하다.

구성 요소가 다른 프로세스에서 실행된다면 IPC 또한 가능하다.(안드로이드 애플리케이션의 각 구성요소는 기본적으로 하나의 스레드/프로세스에서 실행됨에 주의)
때문에 시스템은 두 서로 다른 프로세스 사이의 종속성을 인식하고 프로세스 유지 우선순위를 관리하게 된다.

같은 프로세스 내에서 통신할 경우 Binder 클래스의 확장으로, 여러 프로세스에서 작동할 경우 Messenger와 Handler를 구현하여 통신할 수 있다.
다른 프로세스에서 온 여러 요청을 동시에 처리하는 서비스는 AIDL(Android Interface Definition Language)를 통한 멀티스레딩을 사용할 수 있다.
...
AndroidManifest에 `<receiver>`로 등록하여 정적 등록하거나 구성요소 내에서 `registerReceiver()`를 호출하여 동적 등록할 수 있다. (구성요소의 Context가 유효한 동안 동작한다)
...
`Alarm`,`Notification`,`Widget`등 다른 프로세스가 포그라운드에 있을 때에 Intent를 실행하기 위해 다른 애플리케이션 등에게 Intent의 참조를 열어 놓는 **전달 보류 인텐트**이다.
